...
def generate_figure(area, index, label, specific_provinces):
    ...
    prediction_results1 = pd.read_csv(f'./ALL_AQ/{area}_1{index}.csv')
    prediction_results2 = pd.read_csv(f'./ALL_AQ/{area}_2{index}.csv')
    prediction_results3 = pd.read_csv(f'./ALL_AQ/{area}_3{index}.csv')
    ...
    lon = np.arange(lon_min, lon_max + 0.1, 0.15)
    lat = np.arange(lat_min, lat_max + 0.1, 0.15)
    ...
    # Save figure
    plt.savefig(f'./第五章-图13/{area}_{index}_scatter.png', format='png', transparent=True, pad_inches=0.1, dpi=130,
                bbox_inches='tight')
    # plt.show()
    plt.close()
...
if __name__ == '__main__':
    ...
    for definition in definitions:
        generate_figure(area, definition['index'], definition['label'], specific_provinces)
