...
area = '江苏省'
specific_provinces = ['江苏省']

# Read the country borders and grid static features data
country_borders = gpd.read_file(country_borders_path)
prediction_results1 = pd.read_csv(f'./prediction/{area}_1.csv')
prediction_results2 = pd.read_csv(f'./prediction/{area}_2.csv')
prediction_results3 = pd.read_csv(f'./prediction/{area}_3.csv')
...
cbar.set_label('PM2.5 (µg/m³)', rotation=270, labelpad=12,
               fontsize=8)

# Save figure
plt.savefig(f'{area}_PM2_5_scatter.png', format='png', transparent=True, pad_inches=0.1, dpi=130, bbox_inches='tight')
plt.show()
plt.close()
