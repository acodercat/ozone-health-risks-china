...
results = pd.read_csv("./dataset/o3_pred_4.csv")
results['date'] = pd.to_datetime(results['date'].astype(str), format='%Y%m%d')

min_value = results['pred'].min()

# 获取 'TMP_P0_L1_GLL0' 这一列的最大值
max_value = results['pred'].max()
...
for ax, date in zip(axs, dates):
    ...
    # ax.set_aspect(0.9)
    ax.set_title(format_date(date), fontsize=9)
...
plt.savefig('O3_season_map.png', format='png', transparent=True, pad_inches=0, dpi=130)
plt.show()
