...
prediction_results_path = './dataset/NCP_2021.8.1_actual.csv'
...
norm = mcolors.Normalize(vmin=prediction_results['o3_pred'].min(),
                         vmax=prediction_results['o3_pred'].max())

# Plot interpolated elevation values for specific provinces
ax.scatter(prediction_results['lon'], prediction_results['lat'], c=prediction_results['o3_pred'],
...
# Save figure
# plt.savefig('NCP_actual_scatter.png', format='png', transparent=True, pad_inches=0, dpi=130, bbox_inches='tight')
plt.show()
plt.close()
